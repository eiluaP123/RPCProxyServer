...
//	printf("current empty %d, cache size %d, current cache size %d\n",curr_empty, cache_size, curr_cache_size);
	...
	if (len_data < curr_empty){
		...
		q.push(url);
//		printf("this url is %s and end of the queue is %s\n",url.c_str(),q.back().c_str());
		printf("number of elements in map %d\n",cache_map.size());
	}
	else {
		while(len_data > curr_empty) {
			string get_url = q.front();
...
//			printf("size deleted %d and len of data to be added %d\n", arr.size(), len_data);
			...
			cache_map.erase(itr);
//			printf("this url is %s and front of the queue is %s\n",url.c_str(),q.front().c_str());
			q.pop();
//			printf("this url is %s and front of the queue now is %s\n",url.c_str(),q.front().c_str());
//			printf("number of elements in map %d\n",cache_map.size());
